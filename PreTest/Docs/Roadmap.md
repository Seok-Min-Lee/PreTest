...
> 
- **주요 작업:**
    - `SaveManager`/`LoadManager` 구현: JSON 직렬화(`JsonUtility`)를 통한 거울 배치 정보 `Save`/`Load` 연동. — ✅ `SaveManager`(버튼 클릭 시 즉시 저장)와 `LoadManager`(`Init` 부트스트랩 씬에서 앱 실행 시 1회 자동 로드)로 구현. `Clear`는 별도 작업 예정.
